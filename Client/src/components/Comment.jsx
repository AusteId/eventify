...
const Comment = props => {
  ...
  return (
    <>
      ...
      >
        <div
          className={`relative card-body p-4 rounded-2xl ${isDarkMode ? 'bg-slate-600/40' : 'bg-light-gray'}`}
        >
          {adminRole && props.contextId != props.userId && (
            <button
              onClick={secureClick}
              className="text-error absolute right-[6%] top-[12%]"
            >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between">
                <div>
                  <h3
                    className={`text-body-m font-inter capitalize font-bold ${isDarkMode ? 'text-[#f59e0b]' : 'text-header-dark'}`}
                  >
                    {props.name}
                  </h3>
                  <p
                    className={`font-inter ${isDarkMode ? 'text-amber-700' : 'text-header-dark'} text-body-s opacity-70`}
                  >
                    {props.time}
                  </p>
                </div>
                ...
                {props.contextId == props.userId ? (
                  <div className="dropdown dropdown-end">
                    ...
                    >
                      <MoreVertical
                        className={`duration-750 h-4 w-4 ${isDarkMode && 'text-[#f59e0b] hover:text-gray-200'}`}
                      />
                    </label>
                    <ul
                      tabIndex={0}
                    ...
                    >
                      <li>
                        ...
                        >
                          <Pencil
                            className={`h-4 w-4 ${isDarkMode ? 'text-[#f59e0b]' : ''}`}
                          />
                          Edit
                        </button>
                        <button
              ...
              {editing ? (
                ...
                <p
                  className={`text-body-m font-inter mt-2 break-all overflow-hidden ${isDarkMode ? 'text-gray-200' : 'text-body-medium'}`}
                >
                  {props.comment}
                </p>
              )}
