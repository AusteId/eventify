...
const EventSearch = ({ onSearch }) => {
  ...
  const onSubmit = data => {
    if (
      data.minAge !== '' &&
      data.maxAge !== '' &&
      Number.parseInt(data.minAge) > Number.parseInt(data.maxAge)
    ) {
      setError('minAge', {
        type: 'manual',
    ...
    onSearch({
      ...
      filters: {
        ...
        endDateTime:
          data.startDateTime && data.endDateTime ? data.endDateTime : undefined,
  ...
  const setDateFilter = option => {
    ...
    console.log(
      `Option: ${option}, Start: ${formatDate(startDate)}, End: ${formatDate(endDate)}`,
    );
  ...
  return (
    <div className="mb-6 flex flex-col gap-4">
      <div className="flex flex-col tablet:flex-row tablet:items-center tablet:justify-between gap-4 ">
        <div className="relative w-full mx-auto desktop:w-210 tablet:w-110">
          <button
                    ...
                    <input
                      {...register('city', {
                        pattern: {
                          ...
                          message:
                            'City name can only contain letters, numbers, spaces, dots, or hyphens',
                        },
                      })}
